...
function get_cat_list($__jsonpath,$__mainkey=""){
    $jsonArray = @json_decode(@file_get_contents($__jsonpath),true);
    $catArray = [];
    if(count($jsonArray)>0){
        foreach($jsonArray as $val){
            $catItem= [];
            foreach ($val as $catfield=>$catval){
                $catItem[$catfield] = $catval;
            }
            if($__mainkey!="")
                $catArray[$val[$__mainkey]] =  $catItem;
            else
                array_push($catArray,$catItem);
        }
    }
    return $catArray;
}

// Get the mian category by sub category //
